...
void FSceneRenderer::RenderView(const FSceneView* InSceneView)
{
    ...
    // 렌더링 시작 전, 뷰포트 설정 및 뎁스/스텐실 클리어
    const FRect& ViewRect = InSceneView->GetViewRect();
    CommandList->SetViewport(static_cast<float>(ViewRect.X), static_cast<float>(ViewRect.Y), static_cast<float>(ViewRect.W), static_cast<float>(ViewRect.H), 0.0f, 1.0f);
